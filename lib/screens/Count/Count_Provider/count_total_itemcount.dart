...
class CountTotalIdProvider with ChangeNotifier {
  ...
  Future<dynamic> countTotalIdAPI({required BuildContext context}) async {
    try {
      ...
      var url =
          "$baseApiUrl/seedor-api/warehouse/total-warehouse/count?fields={'cnt_asn','cnt_grn','cnt_del','cnt_stock'}&clientid=$claindId";
      var response = await https.get(Uri.parse(url), headers: headers);
      print(
          "countTotalId-->$baseApiUrl/seedor-api/warehouse/total-warehouse/count?fields={'cnt_asn','cnt_grn','cnt_del','cnt_stock'}&clientid=$claindId");
